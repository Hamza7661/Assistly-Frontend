  ...
  if (!currentApp && apps.length === 0) {
    return (
      <button
        ...
        className="px-3 py-2 text-sm font-medium text-[#c01721] hover:text-[#a8141c] flex items-center gap-2"
      >
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 hover:text-[#c01721] hover:bg-gray-50 rounded-md transition-colors border border-gray-200 hover:border-[#c01721]"
        title={currentApp ? `Current app: ${currentApp.name} (${getIndustryLabel(currentApp.industry)})` : 'Select an app'}
      >
        <Building2 className="h-4 w-4 text-[#c01721]" />
        ...
        {currentApp && (
          <>
            ...
            <span className="hidden xl:block px-2 py-0.5 text-xs font-medium bg-[#c01721] text-white rounded-full">
              Active
            </span>
          </>
        )}
        ...
        <div className="absolute top-full left-0 mt-1 w-64 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200 max-h-96 overflow-y-auto">
          ...
          {apps.length === 0 ? (
            ...
            apps.map((app) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-sm flex items-center justify-between hover:bg-[#c01721]/10 hover:text-[#c01721] ${
                  currentApp?.id === app.id ? 'bg-[#c01721]/10 text-[#c01721]' : 'text-gray-700'
                }`}
              >
                ...
                {currentApp?.id === app.id && (
                  <Check className="h-4 w-4 text-[#c01721] flex-shrink-0 ml-2" />
                )}
              </button>
            ))
          )}
          ...
          <div className="border-t border-gray-200 mt-1">
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-[#c01721] hover:bg-[#c01721]/10 flex items-center gap-2"
            >
              <Plus className="h-4 w-4" />
              Create New App
            </button>
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-[#c01721]/10 hover:text-[#c01721]"
            >
              Manage Apps
            </button>
          </div>
