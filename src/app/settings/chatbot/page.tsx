...
function SortableServicePlanItem({
  ...
  const id = `${leadTypeId}-${servicePlan}`;
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      <div
        {...attributes}
        {...listeners}
        className="cursor-grab active:cursor-grabbing p-0.5 rounded hover:bg-white/20 touch-none"
        title="Drag to reorder"
      >
        <GripVertical className="h-3 w-3" />
      </div>
      <span className="text-xs font-medium max-w-[120px] truncate" title={servicePlan}>
        {servicePlan}
      </span>
      <button
        type="button"
...
export default function ChatbotSettingsPage() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      setError('');
      try {
        const [svc, qSvc] = await Promise.all([
          useIntegrationService(),
          useQuestionnareService()
        ]);
        const [res, servicePlansRes] = await Promise.all([
          svc.getSettings(appId),
          qSvc.list(appId, QuestionnareType.SERVICE_PLAN)
        ]);
        const integration = res.data?.integration;
        const plans = servicePlansRes.data?.faqs || [];
        const validPlanNames = new Set(plans.map((p: any) => p?.question).filter(Boolean));
        setAvailableServicePlans(Array.from(validPlanNames));

        // Set or clear image preview based on integration (clear when new app has no image)
        ...
        const rawLeadTypeMessages = integration?.leadTypeMessages || defaultLeadTypes;
        // Only keep relevantServicePlans that exist in Service Plans (drop stale names like old "Menu Flow")
        const leadTypeMessages = rawLeadTypeMessages.map((lt: LeadTypeMessage) => {
          const relevant = (lt.relevantServicePlans || []).filter((name) => validPlanNames.has(name));
          return {
            ...lt,
            relevantServicePlans: relevant.length > 0 ? relevant : undefined
          };
        });
  ...
  const checkForChanges = (newSettings: IntegrationSettings) => {
    ...
    return (
      ...
      newSettings.chatbotUiMode !== originalSettings.chatbotUiMode ||
  ...
  const handleServicePlanDragEnd = (itemIndex: number, leadTypeId: number, event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    setSettings((prev) => {
      const updated = [...(prev.leadTypeMessages || [])];
      const leadType = updated.find((lt) => lt.id === leadTypeId);
      if (!leadType || itemIndex < 0 || itemIndex >= updated.length) return prev;
      const plans = [...(leadType.relevantServicePlans || [])];
      const oldIndex = plans.findIndex((p) => `${leadTypeId}-${p}` === active.id);
      const newIndex = plans.findIndex((p) => `${leadTypeId}-${p}` === over.id);
      if (oldIndex === -1 || newIndex === -1) return prev;
      const reordered = arrayMove(plans, oldIndex, newIndex);
      updated[itemIndex] = { ...updated[itemIndex], relevantServicePlans: reordered };
      return { ...prev, leadTypeMessages: updated };
    });
          ...
          <div className={`content-wrapper ${isSidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              <NoAppEmptyState
                ...
                description="Create or select an app first to configure chatbot appearance, greeting, and lead type messages."
              />
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => updateSettings({ ...settings, chatbotUiMode: 'basic' })}
                      className={`px-4 py-2 rounded-lg border text-sm font-medium transition ${
                        (settings.chatbotUiMode || 'basic') === 'basic'
                          ? 'brand-chip-selected'
                    ...
                    <button
                      type="button"
                      onClick={() => updateSettings({ ...settings, chatbotUiMode: 'advanced' })}
                      className={`px-4 py-2 rounded-lg border text-sm font-medium transition ${
                        settings.chatbotUiMode === 'advanced'
                          ? 'brand-chip-selected'
                          : 'border-gray-300 text-gray-700 hover:border-gray-400'
                      }`}
                    >
